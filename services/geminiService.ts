...
class GeminiService {
    ...
    async generateClarifyingQuestions(payload: { topic: string, role: string, depth: string, tone: string, focus: string }): Promise<ClarifyingQuestionWithOptions[]> {
    ...
    async generateCourse({ initialTopic, context, depth, role, tone, focus }: CourseCreationPayload): Promise<Course> {
        const prompt = `
        ...
        **INSTRUCCIONES CLAVE DE DISEÑO:**
        ...
        try {
            ...
            const parsedCourse: Course = JSON.parse(response.text);
    ...
    async generateExpandedLesson(course: Course, moduleTitle: string, newLessonTitle: string): Promise<Lesson & { lessonTitle: string }> {
    ...
    async generateComplementaryLesson(courseTitle: string, moduleTitle: string, existingLessonTitles: string[]): Promise<Lesson> {
        const expansionLessonSchema = {
            type: Type.OBJECT,
            properties: {
                lessonTitle: { type: Type.STRING },
    ...
    async generateGuidedLessonStep(course: Course, lesson: Lesson, history: GuidedLessonStep[], userChoice: string): Promise<GuidedLessonStep> {
    ...
    async generateDeeperExplanation(courseTitle: string, lessonTitle: string, lessonContent: string): Promise<string> {
        const prompt = `
    ...
    async simulateCommandExecution(command: string, context: string): Promise<string> {
        const prompt = `
    ...
    async generateFlashcardsForCourse(course: Course): Promise<Flashcard[]> {
    ...
    async getModuleExpansionSuggestions(course: Course, moduleIndex: number): Promise<string[]> {
    ...
    async generateModuleDescription(moduleTitle: string, lessonTitles: string[]): Promise<string> {
        const prompt = `
    ...
    **CONTEXTO DE LA LECCIÓN ACTUAL DEL COLABORADOR:**
    ...
    async generateForumSummary(lessonTitle: string, posts: ForumPost[]): Promise<string> {
    ...
    async generateCertificateData(course: Course, userName: string): Promise<CertificateData> {
        const prompt = `
    ...
    async generateRemedialLesson(incorrectQuestions: { question: QuizQuestion; userAnswer: string }[]): Promise<RemedialLesson> {
        const prompt = `
    ...
    async generateCompetencyFeedback(moduleTitle: string, score: number, total: number): Promise<string> {
        const prompt = `
    ...
    streamExamAttempt(course: Course, history: TutorMessage[]) {
        const prompt = `
    ...
    async generateExamRemediationPlan(courseTitle: string, history: TutorMessage[]): Promise<string> {
         const prompt = `
    ...
    async generateMindMap(course: Course): Promise<MindMapNode> {
        const prompt = `
    ...
    async streamInterviewSimResponse(course: Course, history: InterviewStep[]) {
        const prompt = `
    ...
    async generateNetworkTopology(prompt: string): Promise<NetworkTopology> {
        const schema = {
            type: Type.OBJECT,
            properties: {
                devices: {
